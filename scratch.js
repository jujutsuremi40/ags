...
export default function TeacherPortal() {
  ...
  useEffect(() => {
    ...
    t.assignedClasses = t.assignedClasses || ["Class 9", "Class 10"];
    t.assignedSubjects = t.assignedSubjects || ["Mathematics", "Physics"];
    setTeacher(t);
  ...
  const filteredStudents = students.filter(s => s.cls === selectedClass && (s.name.toLowerCase().includes(searchQuery.toLowerCase()) || s.rollNo.includes(searchQuery)));
  ...
  const saveMarks = () => {
    ...
    showStatus("Marks Saved! Awaiting Admin Release.");
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans text-gray-800">
      {status && (
        <div className={\ixed top-6 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-xl shadow-2xl font-black uppercase tracking-widest text-[10px] flex items-center gap-2 \\}>
          <CheckCircle size={16} /> {status.msg}
        </div>
      )}

      {/* Sidebar */}
      <aside className={\ixed inset-y-0 left-0 z-40 w-64 bg-[#1B1464] text-white transition-transform duration-300 ease-in-out \ md:translate-x-0 md:static flex flex-col\}>
                ...
                <div className="grid lg:grid-cols-2 gap-6 mt-6">
                  ...
                  <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {teacher.assignedSubjects.map((sub: string) => (
                            ...
                            <tr key={student.id} className="hover:bg-gray-50/50">
                              ...
                              <td className="px-6 py-4">
                                <div className="flex items-center justify-center gap-2">
                                  <button onClick={() => handleMarkAttendance(student.id, "present")} className={\px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all \\}>Present</button>
                                  <button onClick={() => handleMarkAttendance(student.id, "absent")} className={\px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all \\}>Absent</button>
                                  <button onClick={() => handleMarkAttendance(student.id, "late")} className={\px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all \\}>Late</button>
                                </div>
                              </td>
                            </tr>
                          );
                  ...
                  <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
                    <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
                      ...
                      <select value={selectedSubject} onChange={e => setSelectedSubject(e.target.value)} className="px-4 py-2.5 rounded-xl border border-gray-200 font-bold text-sm text-[#1B1464] outline-none focus:border-[#00AEEF]">
                        {teacher.assignedSubjects.map((s: string) => <option key={s} value={s}>{s}</option>)}
                      </select>
                    </div>
            ...
            {activeTab === "performance" && (
              <div className="space-y-6">
                <div className="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm text-center">
                  ...
                  <h3 className="text-xl font-black text-[#1B1464] mb-2">Class Performance Analytics</h3>
                  <p className="text-sm text-gray-500 font-medium max-w-md mx-auto">Performance charts and detailed class analytics will be visualized here based on released results.</p>
                  <button className="mt-6 px-6 py-2 bg-gray-100 text-gray-500 font-bold text-sm rounded-xl cursor-not-allowed">Coming Soon</button>
                </div>
              </div>
            )}
...
fs.writeFileSync('c:/Users/ABDULRAHEEM/Desktop/AGS/abexsun-school/src/app/portal/teacher/page.tsx', content);
